...
from elvis.utility.elvis_general import repeat_data, transform_data
...
from elvis.config.infrastructure import InfrastructureMixin, TransformerPreloadHandler
from elvis.config.scheduling import SchedulingMixin, SchedulingRealisationHandler
from elvis.config.time import TimeMixin, TimeRealisationHandler
from elvis.config.vehicles import VehicleMixin
from elvis.sched import schedulers
from elvis.utility.elvis_general import create_time_steps


class ScenarioConfig(InfrastructureMixin, VehicleMixin, SchedulingMixin, TimeMixin):
    ...
    def __str__(self) -> str:
        ...
        printout += str(
            "Std deviation of the SOC distribution: " + str(self.std_deviation_soc) + "\n"
        )
        printout += str("Max parking time: " + str(self.max_parking_time) + "\n")
        printout += str(
            "Number of charging events per week: " + str(self.num_charging_events) + "\n"
        )

        if self.disconnect_by_time is True:
            printout += "Vehicles are disconnected only depending on their parking time\n"
        else:
            printout += str(
                "Vehicles are disconnected depending on their SOC and their parking"
                "parking time (what ever comes first\n"
            )
        if self.queue_length is None:
            printout += "No queue is considered.\n"
        else:
    ...
    @staticmethod
    def from_yaml(yaml_str: ConfigDict) -> ScenarioConfig:
        """Create an instance of ScenarioConfig from a yaml str.

        Args:
            yaml_str: (dict): Import from yaml file.
        """
        assert type(yaml_str) == dict, (
            "The data specified in the yaml for creating an instance "
    ...
    def with_std_deviation_park(self, std_deviation_park):
        ...
        assert isinstance(std_deviation_park, (int, float)), (
            "The mean of the std deviation of the parking time must be of type float."
        )
    ...
    def with_std_deviation_soc(self, std_deviation_soc):
        ...
        assert isinstance(std_deviation_soc, (int, float)), (
            "The std deviation of the SOC mustbe of type int or float."
        )
        assert std_deviation_soc >= 0, "The std deviation of the SOC must be bigger than 0."
...
class ScenarioRealisation:
    ...
    def check_input(**kwargs):
        ...
        for key in keys_necessary:
            assert key in kwargs, (
                str(key) + " is missing as an input to create a ScenarioRealisation."
            )
    ...
    def __init__(self, config=None, **kwargs):
        ...
        # start_date, end_date or resolution not in kwargs
            ...
            print(
                "Creating a ScenarioRealisation with a ScenarioConfig requires the"
        ...
        if isinstance(config, ScenarioConfig):
            self.with_emissions_scenario(
                config.emissions_scenario, self.start_date, self.end_date, self.resolution
            )
            ...
            self.with_transformer_preload(
                config.transformer_preload,
                self.start_date,
                self.end_date,
                self.resolution,
            ...
            if config.charging_events is not None:
            ...
            elif self.sample_method is None or self.sample_method == "independent_normal_dist":
                ...
                self.charging_events = events_from_week_arr_dist(
                    ...
                    time_steps,
                    config.num_charging_events,
                    config.mean_park,
                    config.std_deviation_park,
                    config.mean_soc,
                    config.std_deviation_soc,
                ...
                ValueError(
                    "Events can only be sampled from independent weekly arrival "
            ...
            self.with_emissions_scenario(
                kwargs["emissions_scenario"], self.start_date, self.end_date, self.resolution
            )
            self.renewables_scenario = kwargs["renewables_scenario"]
            self.with_opening_hours(kwargs["opening_hours"])
            ...
            if "charging_events" in kwargs:
                ...
                self.charging_events = [
                    ChargingEvent.from_dict(**ce) for ce in kwargs["charging_events"]
                ]
            elif self.sample_method is None or self.sample_method == "independent_normal_dist":
                time_steps = create_time_steps(self.start_date, self.end_date, self.resolution)
                self.charging_events = events_from_week_arr_dist(
                    kwargs["arrival_distribution"],
                    time_steps,
                    kwargs["num_charging_events"],
                    kwargs["mean_park"],
                    kwargs["std_deviation_park"],
                    kwargs["mean_soc"],
                    kwargs["std_deviation_soc"],
                    kwargs["vehicle_types"],
    ...
    def with_transformer_preload(
        ...
        # Handle None case - no transformer preload
        if transformer_preload is None:
            self.transformer_preload = None
            return
        ...
        if isinstance(transformer_preload, pd.DataFrame):  # DataFrame
            ...
            if isinstance(resolution, str):
                date = datetime.datetime.strptime(resolution, "%H:%M:%S")
            ...
            if res_data is not None:
                ...
                transformer_preload = adjust_resolution(
                    transformer_preload, res_data, self.resolution
                )
    ...
    def with_emissions_scenario(
        ...
        msg_alignment_unsuccessful = "Transformer preload could not be aligned to simulationsteps."
        msg_invalid_value_type = (
            "Transformer preload should be of type: pandas DataFrame, "
            "list containing float or int, int, float."
        )
        ...
        if isinstance(emissions_scenario, (float, int)):
            self.emissions_scenario = [emissions_scenario] * num_time_steps(
        ...
        # pandas DataFrame
            ...
            if res_data is not None:
                ...
                emissions_scenario = adjust_resolution(
                    emissions_scenario, res_data, self.resolution
                )

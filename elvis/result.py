...
class ElvisResult:
    ...
    def __init__(self, scenario=None, realisation_file_name=None):
        ...
        if scenario is not None:
            assert isinstance(scenario, ScenarioRealisation), (
                "Result.scenario must be of type ScenarioRealisation"
            )
        self.scenario = scenario

        if realisation_file_name is not None:
            assert self.scenario is not None, (
                "To store the simulated ScenarioRealisation the "
                "scenario must be passed as type of "
                "ScenarioRealisation."
            )
            self.scenario.save_to_disk(r"../data/realisations/" + str(realisation_file_name))

    def store_power_charging_points(
        self, power_charging_points, pos_current_time_stamp, is_last_step
    ):
    ...
    def store_power_storage_systems(
        self, power_storage_systems, pos_current_time_stamp, is_last_step
    ):
    ...
    def aggregate_load_profile(self, num_simulation_steps=None):
        if num_simulation_steps is None:
            assert self.scenario is not None, (
                "If using result.aggregate_load_profile without "
                "passing the number of simulation steps the "
                "field result.scenario must be set to the scenario "
                "realisation."
            )

            num_simulation_steps = num_time_steps(
                self.scenario.start_date, self.scenario.end_date, self.scenario.resolution
            )
        load_profile = []
        power = {}
    ...
    def get_storage_profile(self, num_simulation_steps=None):
        if num_simulation_steps is None:
            assert self.scenario is not None, (
                "If using result.get_storage_profile without "
                "passing the number of simulation steps the "
                "field result.scenario must be set to the scenario "
                "realisation."
            )
    ...
    def total_energy_charged(self, num_simulation_steps=None):
        ...
        # convert to kWh
        energy /= 3600 / self.scenario.resolution.total_seconds()
    ...
    def max_load(self):
        ...
        """
        assert self.aggregated_load_profile is not None, (
            "Before calculating KPIs the aggreagatedload profile must be calculated."
        )
    ...
    def simultaneity_factor(self, infrastructure=None, bins=None, quantile=None):
        ...
        if (infrastructure is None) and (self.scenario is not None):
            ...
            raise ValueError(
                "If no scenario is stored in result an infrastructure has to be passed"
        ...
        if self.aggregated_load_profile is None:
            assert self.scenario is not None, (
                "To calculate the simultaneity_factor the "
        ...
        assert self.aggregated_load_profile is not None, (
            "Before calculating KPIs the aggregatedload profile must be calculated."
        )
        ...
        if (bins is None) and quantile is not None:
            power_hardware_max = self.get_power_charging_points(infrastructure)
        ...
        return list(zip(hist[0], hist[1], strict=False))
    ...
    def electricity_costs_24_variable(self, variable_electricity_rate):
        ...
        Todo:
            ...
            - PV integration
        """
        assert type(variable_electricity_rate) == list, (
            "The variable electricity rate must be of type list."
        )
        ...
        cost_distr = EquallySpacedInterpolatedDistribution.linear(
            list(zip(hour_stamps_costs, variable_electricity_rate, strict=False)), None
        )

        time_stamps = create_time_steps(
            self.scenario.start_date, self.scenario.end_date, self.scenario.resolution
        )
        ...
        for i in list(range(len(load_profile))):
            ...
            seconds = (
                current_time_stamp.hour * 3600
                + current_time_stamp.minute * 60
                + current_time_stamp.second
                + current_time_stamp.microsecond / 1000000
            )
    ...
    def total_emissions(self):
        assert self.scenario is not None, (
            "Scenario in result must be specified in order to calculate the emissions."
        )
        assert self.scenario.emissions_scenario is not None, (
            "There must be a emissions_scenario specified in order to calculate emissions."
        )

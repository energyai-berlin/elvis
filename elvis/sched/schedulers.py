"""Scheduling policies for EV charging infrastructure."""
            ...
            while go_on is True:
                if parent.parent is not None:
                    ...
                    if isinstance(parent, Transformer):
                        max_power_transformer = parent.max_hardware_power(
                            assign_power["cps"], preload
                        )
                        if storage_system is not None:
                            max_power_storage = storage_system.storage.max_discharge_power(
                                power_storage, resolution
                            )
                        ...
                        max_hardware_power = min(
                            max_hardware_power, parent.max_hardware_power(assign_power["cps"])
                        )
                else:
                    go_on = False
...
class DiscriminationFree(SchedulingPolicy):
    ...
    def sort_cps(self, config):
        ...
        state_list.sort(
            key=lambda x: self.state[x]["times_charged"] / time_steps_to_charge_in_a_row
        )
        state_list.sort(
            key=lambda x: round(
                (self.state[x]["times_charged"] / time_steps_to_charge_in_a_row) % 1, 2
            ),
            reverse=True,
        )

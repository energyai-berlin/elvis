    ...
    for __transformer in infrastructure["transformers"]:
        ...
        for __charging_station in __transformer["charging_stations"]:
            charging_station = ChargingStation(
                __charging_station["min_power"], __charging_station["max_power"], transformer
            )

            # Add all charging points of current charging point
            for __charging_point in __charging_station["charging_points"]:
                __charging_point = ChargingPoint(
                    __charging_point["min_power"], __charging_point["max_power"], charging_station
                )
...
def wallbox_infrastructure(
    ...
    """
    # Validate Input
    assert isinstance(num_cp_per_cs, int), (
        "Only integers are allowed for the number of charging points per charging station."
    )
    num_cs = int(num_cp / num_cp_per_cs)
    assert num_cp % num_cp_per_cs == 0, (
        "Only integers are allowed for the number of charging "
    ...
    msg_power_numeric = (
        "The power assigned to the infrastructure elements must be of type int or"
    ...
    msg_min_max = (
        "The min power assigned to a infrastructure node must be lower than the max power."
    )
    ...
    transformer = {
        "charging_stations": [],
        "id": "transformer1",
        "min_power": min_power_transformer,
        "max_power": power_transformer,
    }

    charging_point = {"min_power": min_power_cp, "max_power": power_cp}

    charging_station = {"min_power": min_power_cs, "max_power": power_cs, "charging_points": []}
